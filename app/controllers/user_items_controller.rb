class UserItemsController < ApplicationController
  ...
  def index
    @user_items = Item.order('code ASC').where.not(status: "unavailable").page(params[:page]).per(10)
    if params[:search].present?
      # regex to handle search using coma/ spaces
      search_terms = params[:search].split(/[,\s]+/)
      @user_items = @user_items.where("lower(name) LIKE :search OR lower(code) LIKE :search OR lower(item_type) LIKE :search OR lower(condition) LIKE :search  OR lower(description) LIKE :search", search: "%#{search_terms.first.downcase}%")
