class AssetLoansController < ApplicationController
  def create
    ...
    formatted_loan_datetime = DateTime.strptime(asset_loan_params["loan_item_datetime"], "%m/%d/%Y %H:%M")
  ...
  def asset_loan_params
      params.require(:asset_loan).permit(:code, :user_id, :loan_item_datetime, :return_estimation_datetime, :necessary, asset_loan_items_attributes: [:asset_loan_id, :item_id, :loan_status, :_destroy])
  end
end
